...
public record UpvotePostCommand(long Id, CurrentUser CurrentUser) : BaseRequest(CurrentUser);

public class UpvotePostHandler : IRequestHandler<UpvotePostCommand>
{
    private readonly ApplicationDbContext _context;

    public UpvotePostHandler(ApplicationDbContext context)
    {
        _context = context;
    }
    
    public async Task<Unit> Handle(UpvotePostCommand request, CancellationToken token)
    {
